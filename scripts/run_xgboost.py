...
from ventas_forecast.paths import DATA_RAW, FIGURES, PREDICTIONS, REPORTS, ensure_output_dirs
...
# Escalar todas las características con robustez frente a outliers
scaler_X = RobustScaler()
X_scaled = scaler_X.fit_transform(X)
X_scaled = pd.DataFrame(X_scaled, columns=feature_cols, index=X.index)

# Transformar la variable objetivo para reducir sesgo y mejorar la precisión
y = np.log1p(df['ventas_futuras'])

# Separación por tiempo/grupo para evitar fuga entre períodos del mismo producto
def group_time_split(df, group_col='CODIGO', test_size=0.2):
    train_idx = []
    test_idx = []
    for _, grp in df.groupby(group_col):
        ...
        if n < 5:
            ...
            continue
        cutoff = int(np.ceil(n * (1 - test_size)))
        train_idx.extend(grp.index[:cutoff])
        test_idx.extend(grp.index[cutoff:])
    return train_idx, test_idx

train_idx, test_idx = group_time_split(df, group_col='CODIGO', test_size=0.2)

X_train = X_scaled.loc[train_idx]
X_test = X_scaled.loc[test_idx]
y_train = y.loc[train_idx]
y_test = y.loc[test_idx]
y_test_orig = df.loc[test_idx, 'ventas_futuras']
groups_train = df.loc[train_idx, 'CODIGO']
...
cv_scores = cross_val_score(best_model, X_train, y_train, groups=groups_train, cv=group_kfold, scoring='r2', n_jobs=1)
print(f"\nValidación Cruzada (GroupKFold) - R² Score: {cv_scores.mean():.4f} (+/- {cv_scores.std():.4f})")

# Gradient Boosting para referencia
...
gbr_cv_scores = cross_val_score(gbr, X_train, y_train, groups=groups_train, cv=group_kfold, scoring='r2', n_jobs=1)
print(f"\nGradient Boosting (GroupKFold) - R² Score: {gbr_cv_scores.mean():.4f} (+/- {gbr_cv_scores.std():.4f})")
...
df['prediccion'] = np.expm1(df['prediccion_scaled'])
...
plt.savefig(FIGURES / 'grafico_clusters.png')
plt.close()
...
plt.savefig(FIGURES / 'grafico_cluster_summary.png')
plt.close()
...
plt.savefig(FIGURES / 'grafico_prediccion.png')
plt.close()
...
plt.tight_layout()
plt.savefig(FIGURES / 'grafico_importancia.png')
plt.close()
...
plt.savefig(FIGURES / 'grafico_evolucion.png')
plt.close()
...
# Ordenndo por fecha más reciente
...
df_reporte_csv.to_csv(PREDICTIONS / 'PREDICCIONES_DETALLADAS.csv', index=False, encoding='utf-8-sig')
print("\n[OK] Reporte detallado guardado: PREDICCIONES_DETALLADAS.csv")

# REPORTE RESUMEN POR CLUSTER
...
# TOP 10 PREDICCIONES (PRODUCTOS MAS IMPORTANTES)
print("\n" + "="*70)
...
top_10_predictions.to_csv(PREDICTIONS / 'TOP_10_PREDICCIONES.csv', index=False, encoding='utf-8-sig')

# RESUMEN EJECUTIVO
resumen_ejecutivo = f"""
{'='*70}
RESUMEN EJECUTIVO - MODELO DE PREDICCIÓN DE VENTAS (XGBoost)
{'='*70}
...
summary_table.to_csv(PREDICTIONS / 'RESUMEN_METRICAS.csv', index=False, encoding='utf-8-sig')
